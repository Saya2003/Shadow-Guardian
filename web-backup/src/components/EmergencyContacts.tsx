...
const EmergencyContacts = () => {
  const [contacts, setContacts] = useState<{name: string, phone: string, status: string, initials: string}[]>([]);
  ...
  return (
    <section className="fade-in-up" style={{ animationDelay: "0.2s" }}>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
        {contacts.length === 0 ? (
          <p className="text-sm text-center text-muted-foreground col-span-full py-6 bg-card border border-border rounded-xl">No emergency contacts configured yet.</p>
        ) : (
          contacts.map((c, i) => (
            <div
              key={i}
