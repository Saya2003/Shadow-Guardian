...
const NearbyGuardians = () => {
  const { nearbyNodes: guardians } = useMeshNetwork();
    ...
    <section className="fade-in-up" style={{ animationDelay: "0.1s" }}>
      ...
      <div className="flex flex-col sm:flex-row items-center gap-6">
        ...
        <div className="flex-1 w-full space-y-2">
          {guardians.length === 0 ? (
            <p className="text-sm text-center text-muted-foreground p-4 bg-surface rounded-lg border border-border">No guardians in range.</p>
          ) : (
            guardians.map((g, i) => (
