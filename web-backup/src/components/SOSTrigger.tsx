...
const SOSTrigger = () => {
  ...
  return (
    <section className="flex flex-col items-center gap-6 py-8 fade-in-up">
      {/* SOS Button */}
      <button
        onClick={!isBroadcasting ? handleTrigger : undefined}
        className={`
      ...
      >
        ...
        <span className="text-base sm:text-lg tracking-wider text-center">
      ...
      {/* Broadcast Status */}
      {isBroadcasting && (
        <div className="flex flex-col items-center gap-2 text-sos fade-in-up mt-2 font-medium">
          <RadioTower className="w-5 h-5 animate-pulse" />
          <p className="text-sm tracking-wide text-center">{broadcastStatus}</p>
        </div>
      )}

      {/* Slide to Cancel */}
      {isBroadcasting && (
        <div className="w-full max-w-xs fade-in-up mt-4">
