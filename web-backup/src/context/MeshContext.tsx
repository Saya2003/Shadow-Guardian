...
export const MeshProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const [activeAlerts, setActiveAlerts] = useState<MeshAlert[]>([
    {
      id: "1",
      type: "warning",
      message: "Unlit alleyway reported near Station Road. Avoid area.",
      time: "2 hours ago",
      hops: 3
    },
    {
      id: "2",
      type: "info",
      message: "Safe space added: 24/7 Pharmacy on Main St.",
      time: "1 day ago",
      hops: 5
    }
  ]);
    ...
    const interval = setInterval(() => {
      setNearbyNodes(current => {
        let updated = current.map(node => {
          ...
          const angDrift = (Math.random() - 0.5) * 15;
          const newSignal = Math.max(0, Math.min(100, node.signal + sigDrift));
          
          return {
            ...node,
            signal: newSignal,
            angle: (node.angle + angDrift) % 360,
            distance: `${Math.floor(30 + (100 - newSignal) * 0.5)}m`
          };
        }).filter(n => n.signal > 15); // Drop weak nodes

        // Randomly discover new nodes occasionally
  ...
  const broadcastSOS = useCallback(() => {
    ...
    // Hop Sequence Simulation
    setTimeout(() => {
      setBroadcastStatus(`Hopping via ${nearbyNodes[0]?.name || "Nearest Node"}...`);
    }, 1500);

    setTimeout(() => {
      setBroadcastStatus("Hop 2 successful: ISP Relay Hit");
    }, 3500);
    ...
    setTimeout(() => {
      ...
      setActiveAlerts(prev => [{
        id: `sos_${Date.now()}`,
        type: "sos",
        message: "Your SOS signal was actively relayed via 2 mesh nodes.",
        time: "Just now",
        hops: 2
      }, ...prev]);
    }, 5500);
